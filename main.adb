...
procedure Main is
  ...
  type BallArray is Array (1..BufferSize) of Ball_Sensed;

  protected BallBuffer is
    entry Put;
    entry Get(B : out Ball_Sensed);
    procedure ClassifyLastAs(B : Ball_Sensed);
  private
    Buffer : BallArray;
  ...
  protected body BallBuffer is
    ...
    begin
      Last_Put := (Last_Put mod BufferSize) + 1;
      Buffer(Last_Put) := Unknown;
      Curr_sz := curr_sz + 1;
    end Put;

    entry Get(B : out Ball_Sensed) when curr_sz > 0 is
    begin
      B := Buffer(Next_out);
    ...
    procedure ClassifyLastAs(B : Ball_Sensed) is
    begin
      Buffer(Last_Put) := B;
    end ClassifyLastAs;
  ...
  begin
    ...
    loop
      ...
      Text_IO.Put_Line("Sensed ball " & Ball_Sensed'Image(Sensed));

      if Sensed = Metal then
        BallBuffer.ClassifyLastAs(Metal);
      elsif Sensed = Unknown then
        BallBuffer.Get(Last_Ball);

        if Last_Ball = Metal then
